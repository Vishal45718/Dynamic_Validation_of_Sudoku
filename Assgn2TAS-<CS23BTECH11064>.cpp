...
void TAS_unlock(int id)
{
    auto exit_start_time = high_resolution_clock::now();
    if (lock.test_and_set(memory_order_acquire))
    {
        lock.clear(memory_order_release);
    }

    auto exit_end_time = high_resolution_clock::now();

    double exit_duration = duration_cast<microseconds>(exit_end_time - exit_start_time).count();
    exittimes.push_back(exit_duration);

    worstexittime = max(worstexittime, exit_duration);

    output_file << "Thread " << id << " leaves CS at " << current_time() << "\n";
}
...
void *check_sudoku(void *arg)
{
    ...
    while (sudoku_valid.load())
    {
        ...
        else
        {
            ...
            TAS_lock(id);
            output_file << "Thread " << id << " completes checking of column " << (task + 1)
                        << " at " << current_time() << " hrs and finds it as ";
            if (valid)
            {
                output_file << "valid.\n";
